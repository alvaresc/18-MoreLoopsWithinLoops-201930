...
def run_test_largest_number():
    ...
    # Test 4:
    expected = 44
    answer = largest_number(([1,2,3,44], [0,0,0], [1]))
    print('Expected and actual are:', expected, answer)
...
def run_test_largest_negative_number():
    ...
    # Test 1:
    expected = -1
    answer = largest_negative_number([(-1, 1, 4),
                             (13, 10, 11, 7, 10),
                             [1, 2, 3, 4]])
    print('Expected and actual are:', expected, answer)
            ...
            for j in range(len(seq_seq[k])):
                if seq_seq[k][j] < 0:
                    ...
                    if enter == 1:
                        ...
                        if seq_seq[k][j] > seq_seq[max_k][max_j] and seq_seq[k][j] < 0:
                            max_k = k
                            max_j = j
    if enter == 0:
        return None
...
def run_test_first_is_elsewhere_too():
    ...
    print('Test 19:continues test 18:')
    expected = False
    ...
    print('Test 20 (continues test 18)')
    expected = True
...
def first_is_elsewhere_too(seq_seq):
    ...
    for i in range(1,len(seq_seq)):
        for k in range(len(seq_seq[0])):
            test_value = seq_seq[0][k]
            for j in range(len(seq_seq[i])):
                if seq_seq[i][j] == test_value:
                    return True
    return False
